<template>
  <div class="ebl-page-container">
    ...
    <div class="ebl-card-container">
      ...
      >
        <VCard width="800" class="ebl-dialog__card ebl-dialog--envelope-detail">
          <VCardTitle class="ebl-dialog__header">
            <span class="title">Envelope Detail</span>
            <VSpacer />
        ...
        <VSlideXReverseTransition leave-absolute>
          ...
          <VCard v-if="dialog.preview" width="600" class="ebl-dialog__card">
            <VCardTitle class="ebl-dialog__header">
...
const pageInfo = ref({
  title: 'Incoming',
  breadcrumbItems: [{ text: 'Menu' }, { text: 'Documents' }, { text: 'Incoming' }],
})
...
const statusTabItems = computed(() =>
  transactionStatusOptions.map((opt) => ({ label: opt.label, value: opt.value })),
)
...
const generateDummyData = () => {
  ...
  const transactionStatus = ['AMENDMENT', 'PAPER', 'DELIVERY']
  const routingData = [
    {
...
const filteredRowData = computed(() => {
  return rowData.value.filter((row) => {
    ...
    const matchesStatus = statusTab.value === '_all' || row.transactionStatus === statusTab.value
    ...
    return matchesCompanyName && matchesBlNo && matchesStatus && matchesDate
  })
})
...
const historyList = ref([
  {
    company: 'Hyundai Merchant Marine Company Limited',
    name: 'Gil-Dong Hong',
  ...
  {
    company: 'HMM',
    name: 'Christopher Jonathan Alexander Robertson',
  ...
  {
    company: 'CJ Logistics International Corporation Branch Office',
    name: 'Maximilian Theodore Harrison',
